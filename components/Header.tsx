...
import { Play, Bell, BellRing, MessageSquare, Volume2, VolumeX, ShieldAlert } from 'lucide-react';
    ...
    <header className="bg-slate-900/80 backdrop-blur-md border-b border-white/5 px-4 pt-[calc(0.5rem+env(safe-area-inset-top))] pb-4 flex items-center justify-between shadow-sm z-40">
      ...
      >
        <div className={`p-2 rounded-xl flex items-center justify-center shadow-lg transition-all duration-500 ${
          isAdmin ? 'bg-red-500 shadow-red-500/40' : 'bg-[#FFCC00] shadow-[#FFCC00]/40'
        }`}>
          {isAdmin ? (
        ...
        <div>
          ...
          <div className="flex items-center gap-1.5 mt-1">
            <span className={`w-1.5 h-1.5 rounded-full animate-pulse ${isAdmin ? 'bg-red-500 shadow-[0_0_8px_red]' : 'bg-green-500 shadow-[0_0_8px_#22c55e]'}`}></span>
      ...
      <div className="flex items-center gap-2">
        ...
        >
          {notifStatus === 'granted' ? <BellRing size={20} /> : <BellRing size={20} className="opacity-30" />}
        </button>
      </div>
    </header>
  );
